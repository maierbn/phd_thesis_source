...
% --------------
%-----
\section{Parallel Scaling of the EMG Model Using High Performance Computing}
...
The results show perfect weak scaling properties of the 0D and 1D solvers, given by the yellow and red lines. Both is expected due to the construction of the algorithm and the parallel partitioning. The 0D problems are \say{embarassingly parallel} and are solved independently of each other. In the 1D problem solver, the values are  transferred to a dedicated process, where the serial Thomas algorithm is employed. Thus, the solution of all 1D problems is also performed independently of each other, but an additional communication step is required.
...
\subsection{Analytic and Numeric Computations of the Jacobian}\label{sec:analytic_numeric_jacobian}
...
The reference measurement is given by using the GMRES solver without any preconditioner, given by the blue lines.
...
Noch mehr die Löser beschreiben?
...
% multidomain: evaluate number of fibers and multidomain mesh width and number of MUs
