...
We use a Intel Core i9-10980XE processor with 18 cores, base frequency of $\SI{3}{\giga\hertz}$, maximum boost frequency of $\SI{4.8}{\giga\hertz}$, cache sizes of 
...
While the \code{simd} scenario relies on the auto-vectorization capabilities of the compiler, the \code{vc} scenarios, which are considered next, explicitely employ vector instruction, abstracted by the \emph{Vc} and \emph{std-simd} libraries. 
